#include <stdio.h>
#include <stdlib.h>
#include <limits.h>
#include <float.h>

unsigned char reverse(unsigned char b) {
   b = (b & 0xF0) >> 4 | (b & 0x0F) << 4;
   b = (b & 0xCC) >> 2 | (b & 0x33) << 2;
   b = (b & 0xAA) >> 1 | (b & 0x55) << 1;
   return b;
}

unsigned char reverse_byte(unsigned char x)
{
    static const unsigned char table[] = {
        ,
    };
    return table[x];
}

int main()
{
    printf("Bits in a char: %+d\n", CHAR_BIT);
    printf("Unsigned Character Max: %+d\n", UCHAR_MAX);
    printf("Unsigned Character Minimum: 0\n");
    printf("Signed Character Maximum: %+d\n", SCHAR_MAX);
    printf("Signed Character Minimum: %+d\n", SCHAR_MIN);
    printf("Unsigned Integer Maximum: %u\n", UINT_MAX);
    printf("Unsigned Integer Minimum: 0\n");
    printf("Signed Interger Max: %+d\n", INT_MAX);
    printf("Signed Integer Minimum: %+d\n", INT_MIN);
    printf("Unsigned Short Maximum: %+d\n", USHRT_MAX);
    printf("Unsigned Short Minimum: 0\n");
    printf("Signed Short Maximum: %+d\n", SHRT_MAX);
    printf("Signed Short Minimum: %+d\n", SHRT_MIN);
    printf("Unsigned Long Maximum: %lu\n", ULONG_MAX);
    printf("Unsigned Long Minimum: 0\n");
    printf("Signed Long Maximum: %+ld\n", LONG_MAX);
    printf("Signed Long Minimum: %+ld\n", LONG_MIN);
    printf("Float Min: %g\n", FLT_MIN);
    printf("Float Max: %g\n", FLT_MAX);


    /*char c;
    c>='A'&&c<='Z'
    c>='a'&&c<='z'
    c>='0'&&c<='9'
    c=='\t'||c=='\n'||c==' '*/

    // char c = ''; there is no empty character. Use null instead

    // printf("%d\n", c);


    unsigned char test = '+';
    char test2 = '+';

    if(test == test2) // for Ascii characters no problem because codes are between 0-127 (see MIX/MAX of char and unisgned char)
    {
        printf("%d\n", (int)test);
        printf("%d\n", (int)test2);
        printf("Nice\n");
    }

    int val=0xCAFE;
    int bits=val&0xF; /*take last four bits*/

    // bits = 0xE;

    if(bits==0x7 || bits==0xB || /*greater than*/ (bits>=0xD))
    {
        printf("Ola good\n");
    }

    val = ((0xFF & val) << 8) | (val>>8);
    // or val = ((0xFF00 & val) >> 8) | ((0xFF & val) << 8);
    //unsigned char pf = 5;
    printf("%x\n", (0xFF00 & 0xCAFE));
    printf("%x\n", (0xFF & 0xCAFE));
    printf("%x\n", val);
    printf("%x\n", 200);
    printf("%x\n",reverse_byte(200));


    val = 0xCAFE;
    val = (val >> 4) | ((val&0xF)<<12);
    printf("%x\n", val);

    return 0;
}
